...
def getBranchName(directory):
    ...
    return kafkasubprocess.check_output(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=directory).strip()

def getRemotes(directory):
    """Returns list of remote git repositories"""
    gitRemoteOutput = kafkasubprocess.check_output(['git', 'remote', '-v'], cwd=directory)
    remotes = []
...
def gitLogValue(format,directory):
    ...
    return kafkasubprocess.check_output(["git", "log", "-1", "--pretty=format:%" + format], cwd=directory).strip()
...
def main(argv):
    ...
    for gcdaFile in gcdaAllFiles:
        ...
        kafkasubprocess.check_call(["gcov", "-p", "-o", gcdaDirectory, gcdaFile], cwd=COV_PATH)
